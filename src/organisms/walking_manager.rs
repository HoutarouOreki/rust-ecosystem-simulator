...
pub struct WalkingManager {
    ...
    idle_time_left: Option<f32>,
    rng: rand::rngs::ThreadRng,
}

const WALKING_SPEED_PER_SECOND: f32 = 128f32;

const IDLE_TIME: [f32; 2] = [3.0, 7.0];

const NEW_TARGET_DISTANCE: [f32; 2] = [150.0, 483.0];
...
impl WalkingManager {
    ...
    pub fn simulate_and_calculate_new_pos(
        ...
        if let Some(target) = self.target {
            ...
            let new_pos = calculate_position(delta_s, current_pos, target);
            ...
            if new_pos.eq(&target) {
                ...
                self.idle_time_left =
                    Option::Some(self.rng.sample(Uniform::new(IDLE_TIME[0], IDLE_TIME[1])));
            }
            new_pos
        } else if let Some(idle_time_left) = self.idle_time_left {
            println!("Idle time left: {}", idle_time_left);
            self.handle_idle_time(delta_s, idle_time_left, current_pos);
            current_pos
        } else {
            self.idle_time_left =
                Option::Some(self.rng.sample(Uniform::new(IDLE_TIME[0], IDLE_TIME[1])));
    ...
    fn handle_idle_time(&mut self, delta_s: f32, idle_time_left: f32, current_pos: Point2<f32>) {
        let idle_time_left_decremented = idle_time_left - delta_s;

        if idle_time_left_decremented <= 0. {
            self.idle_time_left = Option::None;
            self.pick_new_target(current_pos);
        } else {
            self.idle_time_left = Option::Some(idle_time_left_decremented);
        }
    }
...
fn calculate_position(delta_s: f32, current_pos: Point2<f32>, target: Point2<f32>) -> Point2<f32> {
    ...
    if distance <= WALKING_SPEED_PER_SECOND * delta_s {
        ...
        let direction_to_target_per_time =
            vecmath::vec2_scale(direction_to_target, WALKING_SPEED_PER_SECOND * delta_s);
